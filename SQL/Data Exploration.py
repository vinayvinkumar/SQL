...
cursor = conn.cursor()
pop_avg = cursor.execute("select avg(population) from facts;").fetchall()
pop_growth_avg = cursor.execute("select avg(population_growth) from facts;").fetchall()
birth_rate_avg = cursor.execute("select avg(birth_rate) from facts;").fetchall()
death_rate_avg = cursor.execute("select avg(death_rate) from facts;").fetchall()
...
import sqlite3
conn = sqlite3.connect("factbook.db")
projected_population = conn.execute("select round(population + (population * (population_growth/100))) from facts where (population is not null and population_growth is not null) and (population>0 and population<7000000000);").fetchall()
